...
const AppNavigator = () => {
  ...
  useEffect(() => {
    const checkAppState = async () => {
      try {
        ...
        const screenTimeStatus = await AsyncStorage.getItem(SCREEN_TIME_PERMISSION_KEY);
  ...
  const updateScreenTimePermission = async (status) => {
    try {
      await AsyncStorage.setItem(SCREEN_TIME_PERMISSION_KEY, status ? 'true' : 'false');
  ...
  const updateWalletConnection = async (status) => {
    try {
      await AsyncStorage.setItem(WALLET_CONNECTED_KEY, status ? 'true' : 'false');
      ...
      <Stack.Navigator screenOptions={{ headerShown: false }}>
        {!isLoggedIn ? (
          ...
          <Stack.Screen name="Login">
            {props => <LoginScreen {...props} onLoginSuccess={() => updateAuthStatus(true)} />}
          </Stack.Screen>
