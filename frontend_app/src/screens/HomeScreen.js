...
const HomeScreen = ({ navigation }) => {
  ...
  useEffect(() => {
    ...
    const unsubscribe = messaging().onMessage(async (remoteMessage) => {
      ...
      const notificationBody =
        remoteMessage.notification?.body || "새 알림이 도착했습니다";
  ...
  useEffect(() => {
    const loadAllData = async () => {
      ...
      // AsyncStorage에서 오버레이 표시 여부 확인
      try {
        const shouldShowOverlay = await AsyncStorage.getItem(
          "show_welcome_overlay"
        );
        setShowWelcomeOverlay(shouldShowOverlay === "true");
      } catch (error) {
        console.error("오버레이 상태 로드 오류:", error);
        setShowWelcomeOverlay(false); // 오류 시 기본값
      }
      ...
      try {
        ...
        // 병렬로 모든 API 호출
        const [userInfoResponse, pointResponse, focusResponse] =
          await Promise.all([
            fetchUserInfo(),
            fetchUserPoint(),
            fetchDailyFocusTime(),
  ...
  // 오버레이가 표시될 때 호출될 함수 추가 (isLoading이 false로 바뀔 때)
  useEffect(() => {
    // 로딩이 끝나고 오버레이가 표시된 후 AsyncStorage 업데이트
    if (!isLoading && showWelcomeOverlay) {
      const updateOverlayFlag = async () => {
        try {
          await AsyncStorage.setItem("show_welcome_overlay", "false");
          console.log("Welcome overlay flag set to false");
        } catch (error) {
          console.error("AsyncStorage 업데이트 오류:", error);
        }
      };

      // 오버레이가 표시되는 동안 충분한 시간이 지난 후 플래그 업데이트
      const timer = setTimeout(() => {
        updateOverlayFlag();
      }, 1500); // 오버레이가 충분히 보이고 난 후

      return () => clearTimeout(timer);
    }
  }, [isLoading, showWelcomeOverlay]);
  ...
  const checkAppBlockerPermissions = async () => {
    try {
      if (AppBlockerModule) {
        const accessibility =
          await AppBlockerModule.isAccessibilityServiceEnabled();
            ...
            },
            {
              ...
              onPress: async () => {
                try {
                  ...
                  setTimeout(async () => {
                    const newStatus =
                      await AppBlockerModule.hasOverlayPermission();
                ...
                <View style={styles.progressBar}>
                  <View
                    style={[
                      styles.progressFill,
                      {
                        backgroundColor: "#0F51F6",
                        width: `${(userPoint / 480) * 100}%`,
                      },
                    ]}
                  />
                </View>
