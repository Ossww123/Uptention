...
const YOUR_TOKEN_MINT = new PublicKey(
  "5ymZGsCFkfSzZN6AbwMWU2v4A4c5yeqmGj1vSpRWg75n"
);

const APP_URL_SCHEME = "com.anonymous.uptention";

const DEFAULT_PROFILE_IMAGE_URL =
  "https://ddnwvg9t77g5o.cloudfront.net/profile-default.jpg";
...
const ProfileScreen = ({ navigation }) => {
  ...
  // 중앙 크롭된 미리보기 생성
  const createCenteredCropPreview = async (
    imageUri,
    imageWidth,
    imageHeight
  ) => {
  ...
  const handleCurrentPasswordChange = (text) => {
    // 영문, 숫자, 허용된 특수문자(!@#$%^&*)만 입력 가능
    if (text && !passwordRegex.test(text)) {
      setCurrentPasswordError(
        "영문, 숫자, 특수문자(!@#$%^&*)만 사용 가능합니다"
      );
      return;
    } else {
      setCurrentPasswordError("");
  ...
  const handleNewPasswordChange = (text) => {
    // 영문, 숫자, 허용된 특수문자(!@#$%^&*)만 입력 가능
    if (text && !passwordRegex.test(text)) {
      setNewPasswordError("영문, 숫자, 특수문자(!@#$%^&*)만 사용 가능합니다");
      return;
    } else {
      setNewPasswordError("");
    }

    // 비밀번호 패턴 검증 (영문, 숫자 포함, 8~15자)
    const validationRegex = /^(?=.*[A-Za-z])(?=.*\d)[A-Za-z\d!@#$%^&*]{8,15}$/;
    if (text && text.length >= 8 && !validationRegex.test(text)) {
      setNewPasswordError("영문, 숫자가 반드시 포함되어야 합니다");
    } else {
      setNewPasswordError("");
    }
  ...
  const handlePasswordChange = async () => {
    ...
    const validationRegex = /^(?=.*[A-Za-z])(?=.*\d)[A-Za-z\d!@#$%^&*]{8,15}$/;
    if (!validationRegex.test(newPassword)) {
      Alert.alert(
        "오류",
            ...
            <View style={styles.profileInfo}>
              <View style={styles.profileImageContainer}>
                {userInfo?.profileImage ? (
                  <Image
                    source={{
                      uri: `${userInfo.profileImage}?w=105&h=105&t=cover&f=webp`,
                    }}
              ...
              <View style={styles.editModalContent}>
                ...
                <TouchableOpacity
                  style={[
                    ...
                    userInfo?.profileImage === DEFAULT_PROFILE_IMAGE_URL && {
                      opacity: 0.5,
                    },
                  ]}
                  onPress={() => {
